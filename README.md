...
| Web 管理台 | **16 个页面（SPA）** / 31 个路由模块 / 162 端点 |
| 代码规模 | 188 个 `src` Python 模块 / 270+ 测试文件 / 3979 个测试用例（2 skipped）/ 告警中心 Web 页面 |
...
### RAG 知识库
入库格式覆盖 PDF / Word / PPT / 图片 OCR / Markdown / URL / 钉钉文档 / 飞书文档 / 维基空间；BGE 中文模型本地离线推理，混合重排序（向量 0.6 + BM25 0.4）；标题行与正文粘连分块；低于置信度阈值不强行作答，转草稿 / 转人工。另有「**严格问答模式**」开关——开启后回答只能依据知识库内容、未命中直接返回固定「未收录」文案，答案 100% 可溯源、零编造，适合制度问答 / 产品资料这类对出处要求极高的场景。
...
### Web 管理台（`:8080`）
仪表盘统计、对话记录检索、知识库管理、规则引擎配置、草稿审批、在线编辑 `config.yaml`、日志 / 健康检查 / 决策追踪 / 成本质量看板 / **对话摘要实时页**（结构化卡片展示，支持今日 / 昨日 / 近七天时间窗口切换）。

### 智能增强
跨会话长期记忆（按用户隔离、自动压缩归档）、图片 OCR 让 LLM 看懂截图、长对话后台异步摘要压缩、从主人历史消息抽取语气人格、RAG 门控减少 token 浪费、收信探针告警、定时同步 / 备份 / 清理；**主动触达**可在每日定时把近期对话摘要主动推送给主人（钉钉 1:1，默认关闭），并在 Web 后台「对话摘要」页实时查看同源汇总；摘要按日期与主题拆分、以卡片网格呈现，自动提取主题关键词与待办建议，一目了然。
...
proactive   主动触达（每日摘要推送）   rules       黑白名单 / 关键词 / 意图
tools       工具开关与速率限制         skills      技能系统
skillhub    技能市场                  web         管理台与鉴权
storage     默认存储                  logging     日志
safety      安全护栏                  llm_throttle 限流
dead_letter 死信队列                  dws         钉钉 CLI 全局设置
```
