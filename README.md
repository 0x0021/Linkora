...
### 智能增强
跨会话长期记忆（按用户隔离、自动压缩归档）、图片 OCR 让 LLM 看懂截图、长对话后台异步摘要压缩、从主人历史消息抽取语气人格、RAG 门控减少 token 浪费、收信探针告警、定时同步 / 备份 / 清理；**主动触达**可在每日定时把近期对话摘要主动推送给主人（钉钉 1:1，默认关闭），并在 Web 后台「对话摘要」页实时查看同源汇总；摘要以卡片网格呈现，自动提取主题关键词与待办建议，一目了然。
