...
| `embedding` | `enabled` / `provider` / `model` / `top_k` / `base_url` / `api_key` / `hf_token` / `offline` / `device` | BGE 中文向量模型（默认 `bge-small-zh-v1.5`；provider 取值 `local` / `api`，本地 embedding 服务 `http://127.0.0.1:8910/v1`；服务不可达时 RAG 自动降级为不检索）；`device`（`auto` / `cpu` / `mps` / `cuda`，默认 `auto`）控制本地推理设备，重排模型跟随此设置——内存吃紧时改 `cpu` 可避开 macOS MPS 常驻约 1GB 且无法回收的驱动显存，不影响向量维度、无需重建索引 |
