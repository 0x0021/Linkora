...
在既有 `defect-audit.md`（配置/Web安全/轮询/提示词 四类）之外，新增扫描资源/并发/运维/数据生命周期维度，发现 9 项缺陷（D1–D9）。本轮已落地处理 D2/D3（代码）、D4（孤儿会话库图片回收）、D5/D6（回收 4.5G）、D7（全局表保留期清理）、D8（备份内容去重+降保留数）、D9（飞书陈旧索引数据修复）；D1 完成首轮治理（web/routers 静默吞异常 2 处补结构化日志）。此外完成**摘要机制重构**：移除固定 1 小时周期轮询（H2-B `RollingSummaryScheduler`），改为信号驱动的动态摘要（`DynamicSummaryScheduler`）。全部重启验证通过。
...
| **D8** backup churn | `src/db_backup.py` 备份写盘后 SHA-256 比对去重（内容未变更跳过重复备份）+ `backup_max_count` 7→5（模型默认值 + live `config.yaml` 4 处 + `config.yaml.example` 同步）；新增 `tests/test_db_backup.py::TestContentDedup` | 代码已合入（本轮） |
| **摘要机制重构** 移除固定 1 小时周期轮询 | 删除 `src/llm/rolling_summary_scheduler.py`（H2-B）；新增 `src/llm/dynamic_summary_scheduler.py`（信号驱动：静默+新内容 / 体量 / 陈旧 三触发，轻量心跳评估、仅对满足条件的会话调 LLM）；`message_repo` 新增 `get_chats_needing_dynamic_summary` + `collect_dynamic_summary_messages`（收集逻辑改为「自 `last_summary_at` 起的增量」，取代固定时间窗盲取）；`runtime_setup/base/lifecycle` 接线改写；配置 `conversation_summary.rolling`→`dynamic`；新增 `tests/test_dynamic_summary_scheduler.py`(5) | 代码已合入（本轮） |
...
| **D1** `except Exception` 膨胀（src/ 共 309 处 broad-except） | 首轮治理 web/routers 静默吞异常 2 处补结构化日志；本轮 AST 全量扫描 309 处 broad-except，定位「真正静默裸吞」（`pass`/`continue`/`return` 单语句）12 处——逐一核查均为**有意探测**（幂等 DDL、naive/aware 时间相减 TypeError 保守回退、日志净化失败不吞日志本身等），均带注释/`noqa: BLE001` 说明，**非缺陷、按既定策略保留不盲改**。结论：src/ 的 broad-except 是设计内的 fail-soft 探针，批量收窄会破坏既有降级行为，故仅 case-by-case 治理 | 本轮扫描确认（无代码改动）；web/routers 2 处首轮已合入 |
...
- （本轮）refactor(llm): 移除固定 1 小时周期摘要，改为信号驱动动态摘要（DynamicSummaryScheduler + message_repo 收集逻辑 + 配置 rolling→dynamic）+ D8 live/example backup_max_count 7→5
...
## 延后（需设计或配置决策，未动手）
- **D1（后续批次）** —— 经本轮 AST 全量扫描确认：src/ 309 处 broad-except 均为有意 fail-soft 探针，无盲改必要；后续仅对新增代码 / 明确误吞点做 case-by-case 治理。
