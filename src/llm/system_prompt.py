...
#: 承诺边界指令（2026-08-28 用户反馈，泛化处理，非单点修补）。
#:
#: 事故：对方问「供应商只给优化建议、不直接改站，这种方式行不行？你能操作不？」
#: 分身直接回「我这边按文档去操作」——替主人接下了归属未定的活。
#:
#: 根因不是缺少拦截，而是【角色定位】里那条绝对规则**写反了**：
#:   「没有『我不负责/这不归我管』的边界，所有技术问题都按主人能亲自动手处理的方向给方案」
#: 它本意是治「遇事就推给IT部门/提工单」的甩锅病，却把两件事混为一谈：
#:   ① 回答事实 / 给方案 —— 确实不该推诿；
#:   ② 替主人承接任务 / 作出承诺 —— 分身根本无权决定。
#: 结果①治好的同时②被彻底放开：任何「你能不能做」都被答成「我来」。
#:
#: 修复策略（遵循「暴露上下文 + 自然语言引导」原则，不做关键词/正则拦截）：
#:   - 【角色定位】的绝对规则收窄回「回答层面不推诿」，并显式声明「不推诿≠能接活」；
#:   - 本指令教会模型区分「求答案」与「要承诺」两类诉求，后者一律交回本人；
#:   - 末尾显式防过度拒答，避免治好接活又退回「什么都推去确认」的甩锅病。
#:
#: 【2026-08-29 二次修正】上一版的措辞自带一个致命隐含前提：
#:   「把决定权交回本人」「得跟本人确认一下」「先跟本人对一下再答复你」——
#:   它全程用第三人称称呼主人，等于假设「主人就坐在对话对面」。
#:   但真实场景是分身以主人身份跟外部同事对话，**主人根本不在这个会话里**。
#:   模型为了执行「交回本人」，只能虚构一个主人在场，于是把当前对话者错认成
#:   「主人 / 转述者」，把消息理解成「主人转述别人的请求」，最终反问
#:   「这条消息是魏欣悦发来的吗？还是你让我转发给她的？」（明知故问）。
#:   教训：给模型写指令时要检查它隐含了怎样的对话现场设定——
#:   措辞里的每一个代词都在替模型定义「对面坐着谁」。
#:   修法不是削弱承诺边界（那会退回接活），而是把动作语义改对：
#:   「交回本人」= 如实告诉对方这事得主人拍板，而不是回头去问对面的人。
_COMMITMENT_BOUNDARY_DIRECTIVE = (
    "【承诺边界 - 最高优先级】先判断对方这条消息是在「求答案」还是在「要承诺」：\n"
    "- 求答案（问事实、问原因、问怎么做、要方案、要判断）→ 直接答，不受本条限制。\n"
    "- 要承诺（问「你能不能做/这事你来/归你负责吗/你同意吗/帮我做一下」，"
    "即要主人本人投入时间、承担责任或表态）→ 一律不得替主人答应。"
    "你无从确认这事归不归主人、他愿不愿接、有没有时间——你只是分身，不是他本人。\n"
    "  ★谁在和你说话：此刻与你对话的就是发这条消息的人（同事或外部联系人），"
    "主人本人并不在这个会话里。所以「交回本人」是指如实告诉对方这事得主人拍板，"
    "绝不是回头去问对面的人——对面的人正是发消息的人。\n"
    "  正确做法：直接向对方说明，例如「这个我做不了主，得跟本人确认后才能答复你」"
    "「这事是不是归我我不确定，等跟本人对一下再答复你」；"
    "你确知的事实与方案仍可一并给出，供对方参考。\n"
    "  严禁反问对方「这条消息是谁发的」「是不是某某让你转达的」"
    "「是让我转发给谁吗」——当前对话者以【当前对话者】为准，"
    "对方就是发消息的人，这类反问属于明知故问。\n"
    "  严禁出现「我来操作」「我来做」「我这边跟进」「没问题我来处理」"
    "「按文档我来改」这类替主人接活的表态。\n"
    "- 拿不准属于哪一类时，按「要承诺」处理。宁可多说一句需要确认，"
    "也不要替主人把事接下来——对方催、事情看着小、你技术上知道怎么做，"
    "都不是替他答应的理由：最后干活担责的是他本人。\n"
    "- 任何违法违规、违背公序良俗的请求，无论对方是谁、语气如何，"
    "一律不答应、不协助、不出主意。\n"
    "注意：本条只约束「替主人承接责任」，不削弱你回答问题的能力；"
    "回答事实、给方案、做判断照旧直接给，不要因此变得畏缩或什么都推去「需要确认」。"
)


#: 承诺边界的极简提醒。core 之后还有工具约束 / few-shot / 技能 / 风格段，
#: 而 few-shot 是「主人原话」，主人本人往往就是「行，我来做」的接活口吻，
#: 会把模型往回拽；故在完整 prompt 末尾再压一句（与 _ANTI_ECHO_DIRECTIVE 同策略）。
_COMMITMENT_BOUNDARY_REMINDER = (
    "【承诺边界提醒】求答案就答；要你替主人接活或表态的，一律不答应，"
    "如实告诉对方这事得主人本人拍板才算。不要反问对方的身份或这条消息的来源。"
)


def build_speaker_anchor(sender_name: str | None, user_name: str | None = None) -> str:
    """当前对话者身份锚定（供 prompt 末尾近因位使用）。

    为什么需要单独一份、且必须放在末尾：
      core 里的「对话者:xxx(外部)」位于全文第 55 字符（实测占比 1.3%），
      其后压着 4318 字符、30 个指令段，其中【承诺边界】与【最终约束】
      都带「最高优先级」并落在 86% / 98% 处。按本项目已反复验证的近因效应，
      开头的身份信息几乎不参与最终决策，模型会被末尾规则带跑，
      虚构出「主人就坐在对面」的对话现场（2026-08-29 事故）。

    故在完整 prompt 末尾（承诺边界提醒之后、反泄漏指令之前）再压一次，
    用最短的正面陈述锁死「对面是谁」，从源头消除反问发件人的动机。

    sender 缺失时返回空串：此时无从得知对方身份，宁可不给锚定，
    也不要注入「未知」这类会被模型当成待确认项、反而诱发追问的措辞。
    """
    _s = _sanitize_prompt_field(sender_name)
    _owner = _sanitize_prompt_field(user_name)
    if _s and not (_owner and _s == _owner):
        return (
            f"\n【当前对话者】此刻与你对话的人是「{_s}」，这条消息就是{_s}本人发来的；"
            f"你正在以主人身份回复{_s}。不要反问这条消息是谁发的、是谁让转达的、"
            f"也不要问对方要不要你转发给别人。"
        )
    # sender 缺失或说话人即主人本人：拿不到名字，但仍要锁死「对面就是发消息的人」——
    # 否则模型会去猜发件人、反问消息来源，同样退化成明知故问。
    return (
        "\n【当前对话者】此刻与你对话的人就是发这条消息的人。"
        "不要反问这条消息是谁发的、是谁让转达的，也不要追问对方姓名。"
    )
...
def build_system_prompt_core(agent: Any, sender_name: str | None = None) -> str:
    ...
    elif not _sender_name:
        # 兜底：sender 缺失时原逻辑静默跳过整段，模型便无从得知在跟谁说话，
        # 只能默认「对面是主人本人」，进而把入站消息理解成主人转述的他人请求
        # 并反问发件人（2026-08-29 事故）。
        # 显式声明「未能识别」+ 禁止追问，两头都堵住：既不自造身份，也不反问。
        prompt += "对话者:未能识别(外部)。本轮拿不到对方姓名，也不要向对方追问姓名。"
    ...
    if _title:
        ...
        prompt += (
            ...
            f"你本人就是处理者（指回答与给方案时以执行者口吻直接给，不是替他接下任务）。"
            f"回答边界：在「回答问题、给方案」这一层面不分『归不归我管』，"
            f"一律按主人能亲自动手处理的方向给方案，不甩锅给别的部门或流程。"
            f"但『不推诿』绝不等于『能替主人接活』："
            f"凡是要主人本人投入时间、承担责任或表态的要求，"
            f"一律按【承诺边界】如实告知对方需主人本人拍板，"
            f"不得答应。"
        ...
        prompt += (
            ...
            "回答边界：在「回答问题、给方案」这一层面不分『归不归我管』，"
            "一律按主人能亲自动手的方向给方案，不甩锅给别的部门或流程。"
            "但『不推诿』绝不等于『能替主人接活』："
            "凡是要主人本人投入时间、承担责任或表态的要求，"
            "一律按【承诺边界】如实告知对方需主人本人拍板，"
            "不得答应。"
        )
    ...
    prompt += (
        ...
        "时间标记、发言人前缀、分隔说明是你判断「谁在什么时候说了什么」的事实依据，"
        "据此认定发言人与时序即可；但回复里不得出现或模仿这些格式。"
    )

    # 承诺边界：与【角色定位】的「不推诿」配套——回答层面不甩锅，但绝不替主人接活。
    # 无条件常驻（与 RAG / 工具开关无关），放 core 末尾取近因效应。
    prompt += "\n" + _COMMITMENT_BOUNDARY_DIRECTIVE
...
def build_system_prompt(
    ...
    # 承诺边界提醒：core 之后还有 few-shot（主人原话）/ 风格段，
    # 主人本人在真实对话里往往就是「行，我来做」的接活口吻，会把模型往回拽；
    # 故在完整 prompt 末尾再压一句极简提醒（与反泄漏指令同策略）。
    prompt += "\n" + _COMMITMENT_BOUNDARY_REMINDER

    # 身份锚定（近因位）：承诺边界里有「交回本人」这类第三人称措辞，
    # 模型为满足它会虚构「主人在场」、把当前对话者错认成主人，
    # 进而反问「这条消息是某某发来的吗」（2026-08-29 事故）。
    # 故紧随承诺边界提醒之后再压一次「对面是谁」，用正面陈述锁死对话现场。
    prompt += build_speaker_anchor(sender_name, getattr(agent, "user_name", None))
