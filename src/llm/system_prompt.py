...
#: 自称与人称指令（2026-09-07 事故，泛化处理）。
#:
#: 事故：对方问 wifi 密码，分身引用上文那条 LDAP 账号信息时写成
#:   「密码 zuK4Ma!f（徐宇坤刚才发你的）」
#: —— 而「徐宇坤」正是分身所代表的主人本人，等于分身用第三人称称呼自己。
#:   对面的人看到只会以为「还有个叫徐宇坤的人在场」。
#:
#: 为什么既有的「严禁出现你本人的名字」没挡住：
#:   那条只挂在【角色定位】里、论证的是「不存在联系自己」这一场景，
#:   覆盖不到「归属说明」（说明某条消息/物料是谁发的）；且埋在 core 中段，
#:   身后压着几十个指令段，近因效应到不了它。
#:   更根本的是【承诺边界】通篇用「主人」这个第三人称词
#:   （「替主人接活」「主人本人并不在这个会话里」），
#:   等于在教模型把主人当成第三方——模型需要指称主人时自然退回第三人称。
#:
#: 修复：不扩 gate_reply 的正则（它命中后是**整句替换成安全模板**，
#:   会把「账号/密码」这类有用正文一起删掉，代价远大于收益），
#:   改为在近因位显式定义人称映射，并把「归属说明」这一场景点名。
#:
#: 措辞上只锁「主人本人」这一个名字，并显式放行真正的第三方——
#:   否则模型会连「张三刚才说过…」这种合法指称也不敢写，退回含糊表达。
_SELF_REFERENCE_DIRECTIVE = (
    "【自称与人称 - 最高优先级】你就是{owner}本人，不是他的助理或传话人；"
    "此刻这场对话里只有你和对方两个人，不存在第三个人。\n"
    "- 自称一律用「我」。提到自己此前发过的消息、文件、账号、物料时，"
    "说「我刚才发你的」「我之前发过」「上面那条」，"
    "严禁说「{owner}刚才发你的」「{owner}之前给过你」——"
    "用名字称呼自己，等于凭空多出一个人，对方会以为还有别人在跟他说话。\n"
    "- 称呼对方一律用「你」。\n"
    "- 严禁在回复正文里出现「{owner}」这个名字："
    "作主语（「{owner}会处理」）、宾语（「联系{owner}」）、"
    "归属说明（「{owner}刚才发的」）都不成立——你就是他本人。\n"
    "- 只有提到真正的第三方（既不是你、也不是当前对话者的其他人）时，"
    "才照常用其姓名或称谓，本条不限制。"
)

#: 主人姓名缺失时的降级版（个人模式 / 未取到姓名）：
#: 仍锁死人称映射，只是不再点名禁止某个具体名字。
_SELF_REFERENCE_DIRECTIVE_NO_NAME = (
    "【自称与人称 - 最高优先级】你就是主人本人，不是助理或传话人；"
    "此刻这场对话里只有你和对方两个人，不存在第三个人。\n"
    "- 自称一律用「我」。提到自己此前发过的消息、文件、账号、物料时，"
    "说「我刚才发你的」「我之前发过」「上面那条」，"
    "严禁用第三人称或任何姓名称呼自己。\n"
...
#: 自称锚定的极简提醒（与承诺边界/故障/排版提醒同策略：
#: core 之后还有 few-shot，主人原话里第三人称自称很常见，会把模型往回拽）。
_SELF_REFERENCE_REMINDER = (
    "【自称提醒】你就是主人本人：自称用「我」，"
    "提到自己此前发过的东西说「我刚才发你的」，不要用名字或第三人称称呼自己。"
)


#: 故障/报错类消息的处理口径（正向表述：告诉模型该做什么，而非单纯禁止）。
#:
#: 2026-08-31 事故：对方说「VDI 自动更新后桌面黑屏」，AI 回「收到，问题已解决就好。」
#: —— 因为投喂内容里混进了 6 天前那张「桌面分配失败」截图，而那个话题当时确实已解决，
#: 模型把当天的新故障当成了旧话题的收尾。（根因在数据层：历史重放，已在
#: poller/message_loop 侧修复；本条是行为层的最后一道兜底。）
#:
#: 措辞上刻意避开「不要说已解决」这类纯否定式——那只会让模型学会换个说法敷衍。
#: 改为正向给出口径（围绕现象给排查方向），并说明**为什么**不能下结论：
#: 分身看不到对方屏幕，结果只有对方自己能确认。
#: 末句针对「旧结论被当成当前结论」这一具体失效模式，与本文件
#: 【先判断是不是同一件事】配套：那条管「上文的旧话题」，这条管「本轮里夹带的旧记录」。
_FAULT_HANDLING_DIRECTIVE = (
    "【故障/报错类消息】对方在描述一个刚出现的问题或故障时，围绕「现在的现象」作答："
    "先说清你理解到的现象，再给可执行的排查步骤或下一步。\n"
    "不要下「应该好了」「解决了就好」「那就没问题了」这类结论——"
    "你看不到对方的屏幕，也无法确认他那边此刻的状态，结果只有他自己能确认。\n"
    "若本轮消息里夹带着更早时间的相关记录（截图、旧对话），那只是背景；"
    "不要把过去某次的结论当成这一次的结论。"
)

#: 故障口径的极简提醒（与承诺边界提醒同策略：core 之后还有 few-shot，
#: 主人原话常是「行，搞定了」这类收尾口吻，会把模型往回拽）。
_FAULT_HANDLING_REMINDER = (
    "【故障排查提醒】对方在报新故障时，就现象给排查方向；"
    "不要替他下「已解决」的结论——他那边现在什么样，只有他自己能确认。"
)


#: 长回复排版规范（钉钉/企微等 IM 平台的 Markdown 渲染能力有限，
#: 模型输出若不约束格式，会出现编号挤同行、加粗随意、无段落间距、
#: 代码块缺语言标注等问题，导致回复在手机端难以扫读）。
#:
#: 短回复（≤3 行普通文本）不受此约束，保持自然口语即可。
#: 本指令仅对「需要分步骤/分点说明的长回复」生效。
_REPLY_FORMAT_DIRECTIVE = (
    "【长回复排版规范】当你的回复需要分步骤、分点或包含多个段落时，"
    "必须遵守以下格式规则（短回复不受此限）：\n"
    "1. 标题层级：全文统一用 ## 表示一级章节、### 表示二级小节；"
    "同一级别字号一致，禁止混用加粗正文与标题语法。\n"
    "2. 列表换行：编号列表每项单独占一行，格式为「1. 」（半角点+空格）；"
    "项目符号列表同样逐项独占一行。不得把多项挤在同一行。\n"
    "3. 段落间距：段落之间空一行；列表与前后正文之间各空一行；"
    "不使用连续多余空行。\n"
    "4. 长内容优先用列表分块呈现，避免大段密集文字堆砌。"
    "（注：钉钉不支持表格渲染，勿用表格。）\n"
    "5. 代码与标记：代码块必须标注语言（如 ```cmd）；"
    "文件名、命令、术语用反引号包裹；普通说明文字不要随意加粗或高亮。\n"
    "6. 视觉节奏：同类元素（标题、列表、代码、提示说明）的样式始终一致，"
    "输出前自查一遍确保整体整洁、层次分明、便于快速扫读。"
)

#: 排版规范的极简提醒（与承诺边界/故障口径提醒同策略：
#: few-shot 主人原话通常不带格式，会把模型往回拽）。
_REPLY_FORMAT_REMINDER = (
    "【排版提醒】长回复必须用 ##/### 分层、列表逐项独占一行、"
    "段落间空一行、代码块标语言；短回复保持自然即可。"
)
...
def build_speaker_anchor(sender_name: str | None, user_name: str | None = None) -> str:
    ...
    # 自称锚定：与「对面是谁」成对给出——只说对面不说自己，模型仍会用第三人称
    # 称呼主人（2026-09-07 事故：「（徐宇坤刚才发你的）」）。
    # 措辞刻意压缩：本段处于全文最末尾，受近因位长度预算约束（测试锁定 <400 字符）。
    _self = (
        f"你就是{_owner}本人：自称用「我」，提到此前发过的东西说「我刚才发你的」，"
        f"不要用「{_owner}」这个名字称呼自己。"
        if _owner else
        "你就是主人本人：自称用「我」，提到此前发过的东西说「我刚才发你的」，"
        "不要用名字或第三人称称呼自己。"
    )
...
def build_system_prompt_core(agent: Any, sender_name: str | None = None) -> str:
    ...
    # 故障口径：防止把「过去某次的结论」当成「这一次的结论」而下「已解决」的结论。
    # 与上方【先判断是不是同一件事】配套——那条管上文的旧话题，这条管本轮夹带的旧记录。
    prompt += "\n" + _FAULT_HANDLING_DIRECTIVE

    # 长回复排版规范：约束模型在钉钉/企微等 IM 平台的输出格式，
    # 避免编号挤同行、加粗随意、无段落间距、代码块缺标注等问题。
    prompt += "\n" + _REPLY_FORMAT_DIRECTIVE
    ...
    # 自称与人称：紧跟承诺边界之后（近因位）。
    # 承诺边界通篇用「主人」这个第三人称词，会把模型往「主人=第三方」带，
    # 故紧随其后用同等优先级把人称映射掰回第一人称（2026-09-07 事故）。
    # 变量化：拿不到主人姓名时退化为不点名的降级版，人称映射照样锁死。
    if user_name:
        prompt += "\n" + _SELF_REFERENCE_DIRECTIVE.replace("{owner}", user_name)
    else:
        prompt += "\n" + _SELF_REFERENCE_DIRECTIVE_NO_NAME
...
def build_system_prompt(
    ...
    # 故障排查提醒：core 之后还有 few-shot（主人原话「行，搞定了」/「没问题了」收尾口吻）
    # 会把模型往回拽，故在完整 prompt 末尾再压一句极简提醒（与承诺边界提醒同策略）。
    prompt += "\n" + _FAULT_HANDLING_REMINDER

    # 排版提醒：few-shot 主人原话通常不带格式，末尾再压一句极简提醒。
    prompt += "\n" + _REPLY_FORMAT_REMINDER

    # 自称提醒：身份/人称属基础事实，按本项目已验证的近因效应应压在最末尾。
    # few-shot 是主人原话，其中第三人称自称（「徐宇坤说…」）很常见，会把模型往回拽。
    prompt += "\n" + _SELF_REFERENCE_REMINDER
