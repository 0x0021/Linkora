...
class WeatherTool(BaseTool):
    ...
    description = (
        ...
        "天气回复一律用卡片格式输出：以 <card title=\"🌤 {city}天气 · {range}\"> 开头"
        "（{city} 直接用本工具返回的 city 字段——已尽量细到街道/楼栋/地标，不要笼统成'某城市'；{range} 用返回的 range 字段），"
        "卡片正文首行：若本工具返回 note 非空，先写「> ⚠️ {note}」说明已回退到的定位级别，再列当前实况与每日预报。"
        "这样对话里会以专门天气卡片呈现，'定位地点'醒目展示，便于用户一眼看清查的是哪个具体位置。"
    )
